...
import ExcelJS, { Borders, Workbook, Worksheet } from 'exceljs';
...
export class ExcelService {
  ...
  getExcelFromJSONList = async (filename: string, sheets: OneSheet[]) => {
    try {
        const workbook = new Workbook();
        //const workbook = new ExcelJS.Workbook();
        workbook.creator = 'Tarjeta Prepaga | Comafi';
        sheets.forEach(({ name, data }) => this.renderSheet(workbook, name, data));
  ...
  private renderSheet = (wb: Workbook, name: string, data: Array<any>) => {
    if (data.length === 0) throw new Error('Se necesita al menos una fila de datos');
    const planned = this.plainArraysToObjects(JSON.parse(JSON.stringify(data[0])));
    const worksheet = wb.addWorksheet(name, this.sheetConfig);
    this.renderPyramidHeaders('A1', worksheet, planned, `${name.toUpperCase()} SHEET`);
    this.renderData(worksheet, data);
    this.applyStyles(worksheet);
    this.reset();
  };
  
  private renderPyramidHeaders = (cell: string, worksheet: Worksheet, data: any, name: string) => {
    const keys = Object.keys(data);
    const baseKeysCount = this.countBaseKeys(data);
    this.write(worksheet, cell, name);
    worksheet.mergeCells(`${cell}:${this.move(cell, Movements.RIGTH, baseKeysCount - 1)}`);
    cell = this.moveOne(cell, Movements.BOTTOM);
    for (let i = 0; i < keys.length; i++) {
      if (this.isObject(data[keys[i]])) {
        cell = this.renderPyramidHeaders(cell, worksheet, data[keys[i]], keys[i]);
    ...
    if (this.lastAction === Actions.WRITE) cell = this.moveOne(cell, Movements.RIGTH);
  ...
  private renderRow = (cell: string, worksheet: Worksheet, row: Array<any>): string => {
    ...
    row.forEach((column) => {
      if (Array.isArray(column)) {
        ...
        cell = this.moveOne(`${this.getLetter(cell)}${baseRow}`, Movements.RIGTH);
      } else {
    ...
    for (const key in data) {
      if (typeof data[key] == 'object' && data[key]) {
        if (Array.isArray(data[key]) && data[key].length > 0) {
          const allObjects = data[key].filter((e) => typeof e == 'object' && !Array.isArray(e));
          if (allObjects.length == data[key].length) {
            if (allObjects.length == 1) newArray.push(...this.parsePlainArray(data[key]));
            else {
  ...
  private isObject = (data: any) => data && typeof data === 'object' && !Array.isArray(data);
  ...
  private moveOne = (cell: string, movement: Movements) => {
    ...
    if (movement === Movements.BOTTOM && number + 1 > this.lastRow) this.lastRow = number + 1;
  ...
  private applyStyles = (worksheet: Worksheet) => {
    worksheet.columns.forEach((column) => {
        ...
        if (Number(cell.row) <= this.headerFinalRow) {
          if (cell.value && cell.value.toString().length !== 0) {
            if (lastCellWithValue && toMergeCount > 1)
              worksheet.mergeCells(`${lastCellWithValue}:${this.move(cell.address, Movements.BOTTOM, toMergeCount)}`);
      ...
      if (toMergeCount > 0 && lastCellWithValue)
        worksheet.mergeCells(`${lastCellWithValue}:${this.move(lastCellWithValue, Movements.BOTTOM, toMergeCount)}`);
      column.width = maxLength < 10 ? 10 : maxLength;
    });
  };

  private getNumber = (cell: string) => parseInt(cell.substring(cell.search(/\d/)));
